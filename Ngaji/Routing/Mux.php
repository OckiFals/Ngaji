 ...
 * @link https://github.com/c9s/Pux
 * @package Ngaji\Routing
 *
 * Customized and improved by Ocki Bagus Pratama <ocki.bagus.p@gmail.com>
 */
...
class Mux {
    ...
    public function sort() {
        usort($this->routes, array('Ngaji\\Routing\\MuxCompiler', 'sort_routes'));
    }
    ...
    public function dispatch($path) {
        ...
        // Strip / of end of string from Request Url
        if ((HOSTNAME.'/' !== $path) && strpos($path, '/', strlen($path)-1)) {
            $path = rtrim($path, '/');
        }
